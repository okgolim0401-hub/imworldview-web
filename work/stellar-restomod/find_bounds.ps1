...
Write-Host "--- DETAILED MESH BOUNDS ---"
$nodeIdx = 0
foreach ($node in $data.nodes) {
    if ($node.mesh -ne $null) {
        ...
        if ($mIdx -eq 10) { # 앞쪽 라이팅
            ...
            Write-Host "Node: $nodeIdx | Mesh: $meshIdx | PosCenter: ($($wCenterX.ToString('F3')), $($wCenterY.ToString('F3')), $($wCenterZ.ToString('F3'))) | Size: ($($width.ToString('F3')), $($height.ToString('F3')), $($depth.ToString('F3')))"
        }
    }
    $nodeIdx++
}
