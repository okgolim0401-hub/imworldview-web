...
Write-Host "--- SIDE VERTICAL PIXELS ---"
$nodeIdx = 0
foreach ($node in $data.nodes) {
    if ($node.mesh -ne $null) {
        ...
        # Side vertical pixels search space
        if ($wCenterY -gt 0.78 -and $wCenterY -lt 0.98 -and [Math]::Abs($wCenterX) -gt 0.75 -and [Math]::Abs($wCenterX) -lt 0.9) {
            $matName = "default"
            if ($mIdx -ne $null) {
