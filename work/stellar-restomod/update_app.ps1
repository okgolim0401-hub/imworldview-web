try {
    # Read file contents as a single string with correct UTF8 encoding
    $filePath = "app.js"
    $content = [System.IO.File]::ReadAllText($filePath, [System.Text.Encoding]::UTF8)

    # 1. Update isTargetWheel Unicode escape condition
    $oldWheel = "            const isTargetWheel = (s) => {
                if (!s) return false;
                const lo = s.toLowerCase();
                return (lo.includes('wheel') || lo.includes('rim') || lo.includes('disc') || lo.includes('\ud72c')) 
                       && !lo.includes('tire');
            };"
    $newWheel = "            const isTargetWheel = (s) => {
                if (!s) return false;
                const lo = s.toLowerCase();
    ...
    # 2. Update Headlight Glow & Glass Handlers
    $oldHeadlightAndGlass = "                // A. Convert inner headlight glass covers into solid glowing premium horizontal line lights (DRL Bars)
    ...
    $newHeadlightAndGlass = "                // A. Convert inner rectangular glass blocks behind headlights to premium glowing light bars
    ...
    # 3. Update Bumper Pixel Intensity
    $oldBumper = "                if (isBumperPixel) {
                    child.material = child.material.clone();
                    child.material.map = null;
                    child.material.emissiveMap = null;
                    child.material.lightMap = null;
    ...
    # 4. Update Wheels Material Properties
    $oldWheelMat = "                // 4. Targeted Wheels (Metal Gray)
                if (isTargetWheel(rawMat) || isTargetWheel(rawMesh)) {
                    child.material.color.set(0x888888); // Metal Gray
                    child.material.metalness = 0.9;
                    child.material.roughness = 0.4;
                }"
    $newWheelMat = "                // 4. Targeted Wheels (Polished Premium Chrome/Silver Metal)
                if (isTargetWheel(rawMat) || isTargetWheel(rawMesh)) {
                    child.material = child.material.clone();
    ...
    # 5. Remove FilmPass Film Grain Post Processing
    $oldFilm = "        // Film Grain Pass for cinematic noise texture
    ...
    $newFilm = "        const composer = new THREE.EffectComposer(renderer);
        composer.addPass(renderScene);
        composer.addPass(bloomPass);"
