...
function initScrambleEffect() {
    ...
    function scramble(el, delay) {
        ...
        setTimeout(() => {
            ...
            }, staggerDuration + ((length < 30 || originalText.includes('competition') || originalText.includes('Restomod') || originalText.includes('TOKTOK') || originalText.includes('SHADOW') || originalText.includes('BAS RELIEF')) ? 1000 : 1300)); 
    ...
    function triggerSectionSequence(section, sectionIndex) {
        ...
        if (sectionIndex === 2) {
            // Gallery Section (Visualization / 02) is very tall, so paragraphs should observe element-level viewport entry!
            ...
            elements.forEach(el => {
                ...
                // Visualization header label triggers instantly, description paragraphs trigger on view-scroll entry
...
(function initStellaViewer() {
    ...
    if (!container || typeof THREE === 'undefined') {
        ...
        loader.load('stellar_setup_compressed.glb', (gltf) => {
            ...
            const isPaintName = (s) => {
                ...
                // ONLY '차체' is the body paint. '페인트' was used for seats in Rhino.
            ...
            const isTargetInterior = (s) => {
                if (!s) return false;
                const lo = s.toLowerCase();
                // Interior parts that should be orange (Cognac) - Note: '페인트' was used for seats in Rhino
            ...
            model.traverse((child) => {
                ...
                // These were set to '타이어' material or unassigned. By strictly filtering for rawMat === '타이어',
                ...
                if (isLightName(rawMat) || isLightName(rawMesh) || isRearTailLight) {
                    ...
                    if (isRear) {
                        if (yCenter > 0.80) { 
                            ...
                            // Lower vertical bumper lights: premium glossy orange-red (주홍색 레드) plastic reflector
    ...
    function initDiagramSequence() {
        ...
        function triggerTransition() {
            ...
            // then drop ALL lines down together simultaneously ("한번에 쫙")
    ...
    function openLightbox(src, title) {
        ...
        if (currentLightboxIndex !== -1) {
            ...
            if (actualSrc.includes('BAS_main.jpg')) {
                if (toktokLabel) {
                    toktokLabel.style.display = 'block';
                    toktokLabel.innerHTML = '890 x 1520x50 mm<br/>SLS 3D PRINTED NYLON POWDER, XPS FORM';
                }
                if (mergeLabel) mergeLabel.style.display = 'none';
            } else {
    ...
    if (footerGradient && glowWrapper) {
        window.addEventListener('scroll', () => {
            ...
            // Trigger ONLY when the user is within 100px of the absolute bottom!
            // This ensures it stays pure black during banner scroll and shoots up dramatically at the very end.
