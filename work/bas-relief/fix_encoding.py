...
# <img alt="PRODUCTION LOG X" class="w-full h-auto block" src="Bas_test_0X.png"/>
# \s*
# <div class="relative overflow-hidden bg-black">
# We need to insert a closing </div> before the next <div class="relative overflow-hidden bg-black">
# AND one before the </div> closing the grid.
html = re.sub(r'(<img alt="PRODUCTION LOG \d+" class="w-full h-auto block" src="Bas_test_0[4-9]\.png"/>\s*)', r'\1</div>', html)
# Wait, this will add </div> for each img!
...
# Replace the broken texts with the correct ones based on the english text which might not be broken, or by replacing the <p> tags entirely!
def replace_section(section_name, ko_text, en_text, html):
    # Find the p block that starts after <span class="... scramble-text" data-original="OVERVIEW / 01">OVERVIEW / 01</span>
    # Or just replace the <p> tag contents using regex.
    # Since the file is corrupted, the text is broken. We can match the mobile-ko-text and mobile-en-text classes.
    pattern_ko = re.compile(rf'(<span[^>]*data-original="{section_name}[^>]*>.*?</span>\s*<div[^>]*>\s*<div[^>]*>\s*<!-- Mobile Language Switcher -->.*?</div>\s*<p class="[^"]*mobile-ko-text[^"]*">)(.*?)(</p>)', re.DOTALL)
    html = pattern_ko.sub(rf'\1\n      {ko_text}\n      \3', html)
    
    pattern_en = re.compile(rf'(<span[^>]*data-original="{section_name}[^>]*>.*?</span>\s*<div[^>]*>\s*<div[^>]*>\s*<!-- Mobile Language Switcher -->.*?</div>\s*<p class="[^"]*mobile-ko-text[^"]*">.*?</p>\s*<p class="[^"]*mobile-en-text[^"]*">)(.*?)(</p>)', re.DOTALL)
    html = pattern_en.sub(rf'\1\n      {en_text}\n      \3', html)
    return html

html = replace_section("OVERVIEW / 01", ko_01, en_01, html)
html = replace_section("MATERIALITY / 02", ko_02, en_02, html)
html = replace_section("PRODUCTION LOG / 03", ko_03, en_03, html)

with open('C:\\Users\\PC\\Desktop\\IMWV\\WEB\\TokTokShadow_Bas_Relief\\index.html', 'w', encoding='utf-8') as f:
    f.write(html)

print("Fixed encoding and layout!")
