...
# 7. OVERVIEW Section: Clean up the Korean Overview translation text to Bas-Relief context (Removing Stellar name)
$html = $html.Replace('스텔라는 포니와 함께', 'TOKTOK SHADOW / BAS-RELIEF는')
$html = $html.Replace('새로운 디자인 언어와 경험으로 재해석한 스텔라를 제안합니다.', '새로운 디자인 언어와 경험으로 재해석한 바스릴리프 전시를 제안합니다.')
$html = $html.Replace('reinterpreted not as a simple restoration, but as an entirely new design language and experience.', 'reinterpreted not as a simple restoration, but as an entirely new design language and experience for GIAF 2024.')

# 8. Reconstruct the 16 cards grid HTML with line art PNGs (Thumbnail) and 02.jpgs (Lightbox)
...
$template = @'
            <div class="relative aspect-square border border-white/10 bg-black overflow-hidden group/light cursor-pointer" onclick="openLightbox('{lbox}', '{lightId}')">
                <div class="absolute inset-0 border border-white/0 group-hover/light:border-white/20 transition-all duration-300 z-20"></div>
                <img loading="eager" decoding="async" src="{img}" alt="Source Light {indexStr}" class="absolute inset-0 w-full h-full object-contain object-bottom p-0 opacity-60 group-hover/light:opacity-100 group-hover/light:scale-105 transition-all duration-500 z-0" />
                ...
                <div class="absolute inset-0 bg-black/65 opacity-0 group-hover/light:opacity-100 flex items-center justify-center transition-all duration-300 z-30 backdrop-blur-[2px]">
                    <span class="font-label-mono text-[13px] md:text-[15px] font-bold tracking-[0.25em] text-white uppercase translate-y-2 group-hover/light:translate-y-0 transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]">{label}</span>
                </div>
            </div>
'@
...
for ($i = 1; $i -le 16; $i++) {
    ...
    $slotHtml = $template.Replace('{img}', $img)
    $slotHtml = $slotHtml.Replace('{lbox}', $lbox)
    $slotHtml = $slotHtml.Replace('{lightId}', $lightId)
    $slotHtml = $slotHtml.Replace('{indexStr}', $indexStr)
    $slotHtml = $slotHtml.Replace('{label}', $label)
...
# 9. OVERVIEW Section: Remove the entire 3 boxes diagram block from the Stellar template!
...
# 10. Hide transition container to get clean visual flow
...
# 11. Redirect Gallery Image Sources to StellarRestomod
...
# 12. Reconstruct the "NEXT" Project Section
...
# 13. Remove the entire 3D Viewer Section
...
# 14. Fix the copyright symbol with &copy;
...
Write-Host "SUCCESS! Rebuilt clean minimal Overview fully assembled." -ForegroundColor Green
