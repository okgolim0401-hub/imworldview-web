...
$content = $content -replace '(?s)</div>\s*function toggleMobileMenu\(\) \{', "</div>`n`n$lightboxHtml`n`n<script src=`"app_v2.js`"></script>`n<script>`n    function toggleMobileMenu() {"

Set-Content $path $content -Encoding UTF8
Write-Host "Script tag and Lightbox recovered!"
